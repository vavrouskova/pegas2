...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return getSeoDataByUri('page', t('references-organized'));
}

interface OrganizedReferencesPageProps {
...
const OrganizedReferencesPage = async ({ searchParams }: OrganizedReferencesPageProps) => {
    ...
    <main className='max-w-container relative mx-auto'>
      <section className='px-4 sm:px-14'>
        <Breadcrumbs
          ...
          pageTitle={t('references-organized.page-title')}
          items={[{ label: t('references.page-title'), href: `/${t('routes.references')}` }]}
        />
        <PageHeroSection
          title={t('references-organized.hero.title')}
          description={t('references-organized.hero.description')}
        />
      </section>
...
export default OrganizedReferencesPage;
