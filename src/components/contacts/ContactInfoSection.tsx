        ...
        {items.map((item, index) => {
          if (item.type === 'link' && item.href) {
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  'font-text text-[15px] underline hover:no-underline',
                  item.marginTop,
                  item.large && 'text-lg'
                )}
                target='_blank'
          ...
          if (item.type === 'phone' && item.href) {
            return (
              <Link
                key={index}
                href={`tel:${item.href}`}
                className={cn('text-[15px] underline hover:no-underline', item.marginTop, item.large && 'text-lg')}
              >
                {item.content}
              </Link>
            );
          ...
          return (
            <FormattedText
              ...
              text={item.content}
              as='p'
              className={cn('text-[15px]', item.bold && 'font-heading', item.marginTop, item.large && 'text-lg')}
            />
          );
        })}
      </div>
